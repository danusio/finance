#' Matriz de Rendimentos Sincronizados
#'
...
#' gerada uma matriz de retornos (variações de preço) em que cada linha corresponde a uma data
#' de pregão e cada coluna, a um ativo.
#'
#' @param prices_list \code{list} com data.frames no formato "data-preço"
#' @param lag_DC Tempo, em dias corridos, da avaliação dos retornos. O retorno será o de
#' composição contínua (equivalente a \code{TTR::ROC}):
#'
#' \deqn{rcc = ln(P[D+lag_DC]/P[D+0])}
#'
#' @return Matriz com os retornos associados a cada pregão disponível
#' @export
#'
#' @examples
#'
...
#'
#' # 120 dias corridos
#' mat_rend <- genMatRend(lista_precos,120)

genMatRend <- function(prices_list,lag_DC){
  # converte uma lista de preços em uma matriz de rendimentos sincronizados
  # prices_list: lista com data frames date-price
  # lag_DC: lag dos rendimentos, em dias corridos
